...
function updateStats(){
  ...
  const elapsedMin = (Date.now() - wordStart) / 60000; // minutes
...
function handleSubmit(){
  ...
  const user = typeInput.value.trim();
  if(user !== currentWord) {
    scores.player1 -= 10; // or scores[playerTurn] -= 10 in multiplayer
    updateScore();
    message('Incorrect! -10 points.');
    return;
  }
  const elapsed = (Date.now() - wordStart)/1000;
  const pts = calculateScore(currentWord, elapsed);
  ...
  if(mode === 'single'){
    ...
    if(currentIndex + 1 < wordlist1.length){
      ...
      message('');
      stage='result';
      resultText.innerHTML = `You scored <strong class="score">${scores.player1}</strong> points.`;
      showResult();
      saveScore(scores.player1);
      document.getElementById("bestScore").textContent = Math.max(
        parseInt(document.getElementById("bestScore").textContent || 0),
    ...
    if(playerTurn === 1){
      ...
      if(currentIndex + 1 < wordlist2.length){
        ...
        resultText.innerHTML = `Player 1: <strong class="score">${scores.player1}</strong><br/>Player 2: <strong class="score">${scores.player2}</strong><br/><br/><strong>${winner}</strong>`;
        showResult();
        saveScore(Math.max(scores.player1, scores.player2));
        document.getElementById("bestScore").textContent = Math.max(
          parseInt(document.getElementById("bestScore").textContent || 0),
